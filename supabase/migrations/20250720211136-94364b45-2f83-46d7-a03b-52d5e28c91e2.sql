...
-- 1. Fix OTP Expiry Warning (CRITICAL)
-- Set OTP expiry to 1 hour (recommended security threshold)
UPDATE auth.config 
SET mailer_otp_exp = 3600  -- 1 hour in seconds
WHERE id = 'auth.config';

-- 2. Create rate limiting function for anonymous users
...
-- 3. Update RLS policies to include rate limiting for anonymous users
...
-- 4. Add tracking columns to users table for anonymous users
...
-- 5. Create view to monitor anonymous activity
...
-- 6. Function to clean up old anonymous data (GDPR compliance)
