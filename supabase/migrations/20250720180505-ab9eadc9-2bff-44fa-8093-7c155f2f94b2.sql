...
CREATE POLICY "Allow reading own rate limit records" 
ON public.rate_limits 
FOR SELECT 
...
-- Update users RLS policy to include rate limiting
DROP POLICY IF EXISTS "Users can insert own data" ON public.users;
CREATE POLICY "Users can insert own data with rate limit" 
ON public.users 
FOR INSERT 
WITH CHECK (
  auth.uid() = id AND
